...
class Activated extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        if (!$sessionId) {
            ...
            return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('redirectmanager/license/gate');
        }
        ...
        $block = $page->getLayout()->getBlock('redirectmanager.license.activated');
        if ($block) {
            $block->setData('license_key', $licenseKey)
                  ->setData('plan', $planName)
                  ->setData('error', $error)
                  ->setData('settings_url', $this->getUrl('adminhtml/system_config/edit/section/etechflow_redirectmanager'))
                  ->setData('management_url', $this->getUrl('redirectmanager/license/gate'));
        }
